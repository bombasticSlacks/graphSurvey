...
void Fase::expandEnumeration(int depth, int labelNode, long long int label)
{
  ...
  while (vextSz[depth])
  {
    ...
    for(int i = 0; i <= depth; i++) printf("%d ", vext[i][vextSz[depth]]);
    printf("\n");
    /*    
    if (vext[depth][vextSz[depth]] == -1) //stops after completeing the minor stuff
    {
      vextSz[depth]--;
      break;
    }
    */
    if (!sampling || Random::testProb(sampProb[depth]))
    {
      ...
      printf("doing a regular pass on way down depth %d\n", depth);

      expandEnumeration(depth + 1, clabelNode, clabel);
      printf("on the way up depth %d\n", depth);
    }
//Dillons Tests start here
    ...
    else if (!sampling || Random::testProb(sampProb[depth]))
    {
     ...
     // vextSz[depth + 1] = vextSz[depth];
      ...
      for (i = 0; i < eExclNum; i++)
      {
          ...
          for (j = 0; j < depth; j++)
              ...
              if (j == depth)
              {
                  ...
                  printf("succesfully contracted a node\n");
              }
      }
      ...
      printf("recalling expansion depth %d\n", depth);
